...
def login():
    ...
    password_hash = hash_object.hexdigest()[:2]

    # Display the hash value
    messagebox.showinfo(title="Hash Value", message="Generated Hash Value: " + password_hash)
...
# Library App
library_app = Tk()
library_app.title('Library Management System')
library_app.geometry('800x600')
library_app.iconbitmap('src/Icon.ico')
library_app.withdraw()

# Login Page
login_root = Tk()
login_root.title('Login')
login_root.geometry('400x300')
login_root.iconbitmap('src/Icon.ico')

login_canvas = Canvas(login_root, bg='#FFBB00')
login_canvas.pack(fill=BOTH, expand=True)

login_frame = Frame(login_canvas, bg='#FFBB00')
login_frame.place(relx=0.1, rely=0.1, relwidth=0.8, relheight=0.8)

login_label = Label(login_frame, text='Login', bg='black', fg='white', font=('Comic Sans MS', 18))
login_label.place(relx=0, rely=0, relwidth=1, relheight=0.2)

username_label = Label(login_frame, text='Username:', bg='#FFBB00', font=fonts)
username_label.place(relx=0.1, rely=0.3, relwidth=0.3, relheight=0.1)

usernameEntry = Entry(login_frame, font=fonts)
usernameEntry.place(relx=0.4, rely=0.3, relwidth=0.5, relheight=0.1)
usernameEntry.focus()

password_label = Label(login_frame, text='Password:', bg='#FFBB00', font=fonts)
password_label.place(relx=0.1, rely=0.5, relwidth=0.3, relheight=0.1)

passwordEntry = Entry(login_frame, show='*', font=fonts)
passwordEntry.place(relx=0.4, rely=0.5, relwidth=0.5, relheight=0.1)

login_btn = Button(login_frame, text='Login', font=fonts, command=login)
login_btn.place(relx=0.3, rely=0.7, relwidth=0.4, relheight=0.1)


library_app.title("Library App")
library_app.withdraw()
login_root.mainloop()
